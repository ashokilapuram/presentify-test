...
const TextOptions = ({
  ...
  const backgroundTitles = [
    'White', 'Light Gray', 'Sky Blue', 'Purple',
    'Emerald', 'Amber', 'Rose', 'Cyan',
    'Lime', 'Violet', 'Orange', 'Slate'
  ];
      ...
      <div className="option-group">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
          <QuickColorsGrid
            ...
            titles={backgroundTitles}
            onColorSelect={(color) => updateSlideElement(selectedElement.id, { backgroundColor: color })}
          />
          <ColorPickerGroup
            value={selectedElement.backgroundColor || '#ffffff'}
            onChange={(color) => updateSlideElement(selectedElement.id, { backgroundColor: color })}
            onReset={() => updateSlideElement(selectedElement.id, { backgroundColor: undefined })}
            pickerTitle="Pick custom color"
            resetTitle="Remove background color"
          />
        </div>
      </div>
